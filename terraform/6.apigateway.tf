...
#Reports Lambda Integration
resource "aws_apigatewayv2_integration" "api_reports" {
...
resource "aws_lambda_permission" "api_gw_reports" {
...
resource "aws_apigatewayv2_route" "api_reports" {
  api_id    = aws_apigatewayv2_api.api.id
  route_key = "ANY /api/reports"
  target = "integrations/${aws_apigatewayv2_integration.api_reports.id}"
}
